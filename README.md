...
In this tutorial, we will use the same blue LED in Tutorial 1, then only changes happen in this tutorial are the program will be replace for an output of glowing light effect, as image below. Where the blue LED color will start from OFF and slowly glowing until it reach maximum light ON and turn OFF again and repeat the process again and again.
