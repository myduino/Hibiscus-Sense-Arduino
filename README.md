...
As we know, the output signal from Tutorial 1 is `DIGITAL OUTPUT`, where the output signal is either turn ON or turn OFF, but to achieve glowing light effect, we cannot rely on digital output signal program anymore. Therefore we need a signal that can varies range of output signal from minimum to maximum value. What we need is Pulse-Width Modulation (PWM), whereas the signal output that has been generated by PWM, we also called it as **`ANALOG OUTPUT`**.
