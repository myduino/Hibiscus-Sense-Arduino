...
2. **`Middleware`**: The cloud/server, which handle the inbound/outbound of data transmission and communication over the Internet, including data storage, data transfer, data analytics, etc, such as [Favoriot](https://platform.favoriot.com/v2/signup/FAVO7245IFBK), [Blynk](https://blynk.io/), [Thingspeak](https://thingspeak.com/), [ParticleIO](https://www.particle.io/) and more.
3. **`App`**: The software interface, visualize the data from the IoT devices in form of human readable format, alert, notification and analytics interfaces. We can find the app in from of mobile app, desktop app or web app.
4. **`Internet`**. The Internet technology, such as Cellular (GSM/GPRS, 3G, 4G, 5G), Ethernet, Satellite and WiFi.
